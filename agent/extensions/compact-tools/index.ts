...
export default function (pi: ExtensionAPI) {
	...
	> = {
		bash(args, theme) {
			...
			let line = theme.fg("toolOutput", cmd ? truncate(cmd, CALL_ARG_MAX) : "...");
		...
		read(args, theme, context) {
			...
			if (parts.length) line += theme.fg("muted", ` (${parts.join(", ")})`);
			return line;
		},
		grep(args, theme, context) {
			const parts = [
				args?.pattern ? truncate(String(args.pattern), CALL_ARG_MAX) : "...",
			];
			if (args?.path != null)
		...
		ls(args, theme, context) {
			const path = displayPath(args?.path == null ? "." : String(args.path), cwdOf(context));
			let line = theme.fg("toolOutput", path);
	...
	for (const name of Object.keys(factories) as ToolName[]) {
		...
		const tool: AnyDef = {
			...
			renderCall(args: AnyDef, theme: Theme, context: AnyDef) {
				if (density !== "title") return stock.renderCall(args, theme, context);
				return paintedText(
					context,
					theme,
			...
			renderResult(
				...
				return paintedText(context, theme, theme.fg("error", `  ✗ ${msg}`));
			},
		};
		...
		pi.registerTool(
			Object.defineProperty(tool, "renderShell", {
				get: () =>
					density === "title" ? "self" : (stock.renderShell ?? "default"),
				configurable: true,
			}),
		);
	}
